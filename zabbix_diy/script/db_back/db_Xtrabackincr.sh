...
if [ $? -eq 0  ]
then
    echo "恭喜你,${PName}_incrbak_${Now}.sql.gz--数据包已经生成"
else
    ...
    echo "${PName}_incrbak_${Now}.sql.gz--完全备份数据包生成失败" && rm -rf $backup_dir/${PName}_incrbak_$Now.sql.gz  &&  exit
fi
echo 启动远程上传
echo 尝试开始上传
echo 连线正常
...
#数据库备份节点-A区
PUTFILE=${PName}_incrbak_$Now.sql.gz
...
then
	...
	rm -rf $backup_dir/${PName}_incrbak_$Now.sql.gz && echo "数据包删除成功"
	exit
fi
...
rm -rf $backup_dir/${PName}_incrbak_$Now.sql.gz && rm -rf $backup_dir/mysql  && echo 临时文件删除成功!
echo 备份程序已完成!
